...
/// Builds only the native transports selected by Cargo features. The default
/// build does not inspect either libfabric or UMDK installations.
fn main() {
    if env::var_os("CARGO_FEATURE_RDMA").is_some() {
        build_rdma();
    }
    if env::var_os("CARGO_FEATURE_URMA").is_some() {
        build_urma();
    }
}
