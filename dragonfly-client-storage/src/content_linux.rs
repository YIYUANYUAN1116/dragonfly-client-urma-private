...
mod tests {
    ...
    #[cfg(any(feature = "rdma", feature = "urma"))]
    ...
    #[cfg(feature = "urma")]
    #[tokio::test(flavor = "multi_thread")]
    async fn test_write_urma_stream_direct_for_all_piece_kinds() {
        ...
        let persistent_path = content.create_persistent_task(task_id, 10).await.unwrap();
        let (mut reader, recycled) =
            crate::client::urma::UrmaStreamReader::from_test_windows(vec![vec![
                b"urma".to_vec(),
                b"-tail".to_vec(),
            ]]);
        content
        ...
        assert_eq!(
            &tokio::fs::read(persistent_path).await.unwrap()[1..],
            payload
        );

        let cache_path = content
            .create_persistent_cache_task(task_id, 10)
            .await
            .unwrap();
        let (mut reader, recycled) =
            crate::client::urma::UrmaStreamReader::from_test_windows(vec![vec![
        ...
        content
            ...
            .unwrap();
        assert_eq!(recycled.load(Ordering::Acquire), 1);
        assert_eq!(&tokio::fs::read(cache_path).await.unwrap()[1..], payload);
    }

    #[cfg(feature = "urma")]
    #[tokio::test(flavor = "multi_thread")]
    async fn test_write_urma_stream_rejects_length_mismatch() {
        let temp_dir = tempdir().unwrap();
        let config = Arc::new(Config::default());
        let content = Content::new(config, temp_dir.path()).await.unwrap();
        ...
        let (mut overlong, _) = crate::client::urma::UrmaStreamReader::from_test_windows(vec![
            vec![b"0123".to_vec()],
            vec![b"4567".to_vec()],
        ]);
        let error = match content
            ...
            .await
        {
            Ok(_) => panic!("overlong URMA stream must fail"),
            Err(error) => error,
        };
        assert!(error.to_string().contains("exceeded expected length"));

        let (mut short, _) =
            crate::client::urma::UrmaStreamReader::from_test_windows(vec![vec![b"0123".to_vec()]]);
        let error = match content
            .write_piece_from_urma_stream("piece", task_id, 0, 8, &mut short, TEST_WINDOW_TIMEOUT)
            .await
        {
