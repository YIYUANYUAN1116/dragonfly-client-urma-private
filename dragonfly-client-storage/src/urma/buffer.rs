...
mod native {
    ...
    impl UrmaBufferPool {
        /// Test-only pool with no registered Segment and the given slot layout.
        #[cfg(test)]
        pub(crate) fn from_test_slot_states(states: &[(SlotKind, SlotState)]) -> Self {
            Self {
                segment: None,
                slots: states
                    .iter()
                    .enumerate()
                    .map(|(index, (kind, state))| BufferSlot {
                        kind: *kind,
                        offset: index * 64,
                        len: 64,
                        state: *state,
